...
def new_word_add(message):
    words = message.text.split()
    if words[0] not in [i for i in session.query(Joint_Words.target_word).filter(Joint_Words.user_id == message.chat.id).all()]:
...
def delete_word_from_db(message):
    ...
    if session.query(Joint_Words).filter(Joint_Words.target_word == message.text and Joint_Words.user_id == message.chat.id) is not None:
        session.query(Joint_Words).filter(Joint_Words.target_word == message.text and Joint_Words.user_id == message.chat.id).delete()
        session.commit()
        session.close()
...
def add_words_to_Others(word_list):
...
def first_known_word(id):
    first_word = Joint_Words(target_word='Мир', translate='Peace', user_id = id)
    session.add(first_word)
    session.commit()
    session.close()

random_words = ['Elephant', 'Sunset', 'Whisper', 'Galaxy', 'Adventure', 'Umbrella', 'Mystery', 'Rainbow', 'Butterfly', 'Freedom']
add_words_to_Others(random_words)


def create_other_words_list():
    other_words = []
...
def take_target_word(message):
    known_words = []
    for i in [word for word in  session.query(Joint_Words.target_word).filter(Joint_Words.user_id == message.chat.id).all()]:
        known_words.append(i[0])
    return known_words


def take_translate(word):
    translate = [i for i in session.query(Joint_Words.translate).filter(Joint_Words.target_word == word).all()][0][0]
    return translate
